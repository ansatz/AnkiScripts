...
def change_tags(n_days=7, note_types=None):
    ...
    query = f"(tag:__filter__ or tag:__select__ or tag:__question__) (tag:marked or flag:4)".format()
    

    card_ids = mw.col.find_cards(query)
    filtered_count, selected_count, question_count, list_count = 0,0,0,0
    #TODO lambda version
    ...
    for card_id in card_ids: 
        ...
        # filter to list and remove flag:4
        if "__filter__" in note.tags and flag == 4:
            note.delTag("__filter__")
            note.addTag("__list__")
            note.flush()
            card.setUserFlag(0)
            card.flush()
            list_count += 1

        # the tag from __filter__ to __select__ and remove the marked
        elif "__filter__" in note.tags and flag != 4:
            note.delTag("__filter__")
            note.delTag("marked")
        ...
        # the tag from __select__ to __question__ and remove the marked
        ...
        # the tag from __select__ to __filter__ and remove the flag if the card has flag:4
        elif "__select__" in note.tags and flag == 4:
            note.delTag("__select__")
            note.delTag("marked")
        ...
        # question to list if question is marked
        elif "__question__" in note.tags and "marked" in note.tags:
            note.delTag("__question__")
            note.delTag("marked")
            note.addTag("__list__")
            note.flush()
        ...
        # list -> question
        elif "__list__" in note.tags and "marked" in note.tags:
            note.delTag("__list__")
            note.delTag("marked")
            note.addTag("__question__")
            note.flush()
            question_count += 1
    ...
    return filtered_count, selected_count, question_count, list_count
...
def newCards():
    ...
    for card_id in card_ids:
        ...
        # check no tags 
        if bool( set(["__filter__", "__select__","__question__"]) & set(note.tags) ) == True:
            continue
        else:
            note.addTag("__filter__")
...
def on_change_tags():
    ...
    filtered_count, selected_count, question_count,list_count = change_tags(n_days=7)
